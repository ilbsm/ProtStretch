...
class WholeTheory(Whole):
    ...
    def _collect_traces(self):
        return [Theory(self._filename[:2]+str(int(i)).zfill(2) + '.afm', i, **self._kwargs) for i in range(1,
                                                                                                  self._number_of_traces)]

    def _get_total_trace(self):
        return Theory("total.txt", case=None, p=self.p_mean, k=self.k_mean)

    def _analyze_total_trace(self):
        self._total_trace.state_boundaries()
        self._total_trace.plot(name="total_theory")
